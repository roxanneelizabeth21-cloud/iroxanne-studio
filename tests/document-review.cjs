...
let contract={id:'review',status:'sent',access_token:'private-token',client_name:'Review Client',client_email:'review@example.invalid',project_title:'Isolated review',terms:master,price_total:3500,deposit_amount:1750},invoices=[],emails=[];
...
assert.equal(contract.terms,master);assert.equal(contract.signature_consent,true);assert.equal(invoices.length,1);assert.equal(invoices[0].deposit_amount,1750);
