...
export function paymentSummary(invoice: any, payments: any[]) {
  ...
  const balanceLedger = payments.filter(p => p.kind !== 'deposit' && p.kind !== 'project').reduce((n,p) => n + cents(p.amount), 0);
  ...
  const projectPaid = payments.filter(p => p.kind === 'project').reduce((n,p) => n + cents(p.amount), 0);
  const towardDeposit = invoice.deposit_status === 'waived' ? 0 : Math.min(projectPaid, Math.max(0, deposit - legacyDeposit - depositLedger));
  const depositPaid = legacyDeposit + depositLedger + towardDeposit;
  const balancePaid = legacyBalance + balanceLedger + projectPaid - towardDeposit;
