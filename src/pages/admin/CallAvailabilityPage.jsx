...
export default function CallAvailabilityPage(){
 ...
 <div className="space-y-3"><h2 className="font-semibold">Weekly hours</h2>{names.map((name,n)=>{const w=s.weekly_hours.find(x=>x.day===n);return <div key={name} className="flex flex-wrap items-center gap-3"><label className="flex gap-2 w-32"><input type="checkbox" checked={!!w} onChange={e=>update('weekly_hours',e.target.checked?[...s.weekly_hours,{day:n,start:'17:00',end:'19:30'}]:s.weekly_hours.filter(x=>x.day!==n))}/>{name}</label>{w&&<><Input aria-label={name+' opening time'} type="time" step="900" className="w-32" value={w.start} onChange={e=>day(n,{start:e.target.value})}/><span>to</span><Input aria-label={name+' closing time'} type="time" step="900" className="w-32" value={w.end} onChange={e=>day(n,{end:e.target.value})}/></>}</div>;})}</div>
