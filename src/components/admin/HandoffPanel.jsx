...
export default function HandoffPanel({contractId,onSaved}){
 ...
 useEffect(()=>{call('load').then(d=>{setC(d.contract);setPayment(d.payment);}).catch(e=>setError(e.message));},[contractId]);
 const run=async(action)=>{setBusy(true);setError('');try{
   ...
   if(d.contract)setC(d.contract);if(d.payment)setPayment(d.payment);if(d.link)setLink(d.link);if(action==='reopen')setLink('');
   await onSaved();
 ...
 {shareLink&&payment?.paid_in_full&&<div className="space-y-2"><label className="text-sm">Private client handoff link<Input readOnly value={shareLink}/></label><Button variant="outline" onClick={()=>navigator.clipboard.writeText(shareLink).catch(()=>setError('Select and copy the link above.'))}>Copy link</Button><p className="text-xs text-muted-foreground">Share this link with the client when you are ready. Creating it does not email them.</p></div>}
 </>}
 </div>;
}