    ...
    <section id="work" className="mx-auto max-w-6xl border-t border-border px-6 py-20">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
        {(loading ? Array.from({ length: 3 }) : items).map((item, i) => {
          ...
          const cls =
            'group block overflow-hidden rounded-xl border border-border bg-card/50 transition hover:border-primary/40';
          return hasCaseStudy ? (
            <Link key={item?.id || i} to={`/work/${item.slug}`} className={cls}>
              {inner}
            </Link>
          ) : (
            <article key={item?.id || i} className={cls}>
              {inner}
            </article>
          );
        })}
