...
// Step 1 — what the owner is promoting, picked from the saved portfolio.
export default function StepMusic({ draft, patch, portfolioItems, campaigns }) {
  ...
  const choose = (p) => {
    const camps = campaignFor(p.id);
    const active = camps.find((c) => c.status === 'Active') || (camps.length === 1 ? camps[0] : null);
    patch({ portfolioItemId: p.id, campaignId: active ? active.id : '' });
  };
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Pick a project from your portfolio. Everything already stored about it is used automatically.</p>
      </div>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {rows.map((p) => (
          <Card
            key={p.id}
            icon={LayoutGrid}
            selected={draft.portfolioItemId === p.id}
            onClick={() => choose(p)}
            image={p.cover_image_url}
            title={p.title}
            lines={[
              p.category,
              p.date_built ? formatDate(p.date_built) : '',
              campaignFor(p.id).map((c) => c.name).join(', '),
            ]}
          />
        ))}
        {!rows.length && (
      ...
      {selected && (
        <p className="text-xs text-muted-foreground">
          Using your saved details for {selected.title}{selected.description ? ' — description, category and tech included.' : '.'}
        </p>
      )}
      ...
      <div className="space-y-1.5">
        ...
        <Textarea
          ...
          rows={2}
          placeholder='e.g. "Lead with the behind-the-build angle", "Focus on the booking feature", "Do not name the client"'
        />
      </div>
